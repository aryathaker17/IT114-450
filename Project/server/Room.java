...
import Project.common.GenericUtils;
...
public class Room implements AutoCloseable {
	...
	private final static String MUTE = "m";
	private final static String UNMUTE = "u";
	...
	protected synchronized void addClient(ServerThread client) {
		...
		if (clients.indexOf(client) > -1) {
			...
			client.setFormattedName(String.format("<font color=\"%s\">%s</font>", GenericUtils.getRandomHexColor(),
	...
	private boolean processCommands(String message, ServerThread client) {
		...
		try {
			...
			{
				String[] comm = message.split(MUTE);
				String part2 = comm[1];
				String[] comm1 = part2.split(" ");
				String username = comm1[0];
				logger.log(Level.INFO, "Muted Username: " + username);
				wasCommand = true;	
				client.sendMessage(client.getClientId(), message);
				synchronized (clients) {
					Iterator<ServerThread> iter = clients.iterator();
				...
				String[] comm = message.split(UNMUTE);
				String part2 = comm[1];
				String[] comm1 = part2.split(" ");
				String username = comm1[0];
				logger.log(Level.INFO, "Muted Username: " + username);
				...
				synchronized (clients) {
					...
					while (iter.hasNext()) {
						...
						logger.log(Level.INFO, "Checking against username: " + target.getUsername());
						if (username.equals(target.getUsername())) {							
							target.removeMutedUser(username);
							boolean messagesend = target.sendMessage(target.getClientId(), message);
							if (!messagesend) {
								handleDisconnect(iter, client);
							}
	...
	protected synchronized void sendMessage(ServerThread sender, String message) {
		...
		synchronized (clients) {
			...
			while (iter.hasNext()) {
				...
				if(sender.mutedListCheck(message))
				{
	...
	protected String formatMessage(String message) {
		...
		for (Entry<String, String> kvp : converter.entrySet()) {
			if (GenericUtils.countOccurencesInString(alteredMessage, kvp.getKey().toLowerCase()) >= 2) {
			...
			while (iter.hasNext()) {
				...
				if (clientInRoom.getClientId() != receiver.getClientId()) {
					boolean messageSent = receiver.sendExistingClient(clientInRoom.getClientId(),
							clientInRoom.getClientName(), clientInRoom.getFormattedName());
		...
		synchronized (clients) {
			for (int i = clients.size() - 1; i >= 0; i--) {
				...
				boolean messageSent = client.sendConnectionStatus(sender.getClientId(), sender.getClientName(),
						sender.getFormattedName(),isConnected);
				if (!messageSent) {
					clients.remove(i);
