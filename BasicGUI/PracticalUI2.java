...
BoxLayout.Y_AXIS));
        ...
        content.add(Box.createRigidArea(new Dimension(0, 
200)));
...
provided");
            ...
            if (isValid) {
                System.out.println("Chosen username: " + 
username);
...
be triggered");
    ...
    private void createChatScreen() {
        ...
        wrapper.setLayout(new BoxLayout(wrapper, 
BoxLayout.Y_AXIS));
        JPanel content = new JPanel();
        content.setLayout(new BoxLayout(content, 
BoxLayout.Y_AXIS));
...
wraps it
        ...
        input.setLayout(new BoxLayout(input, 
BoxLayout.X_AXIS));
...
calcHeightForText(text, content.getWidth())));
...
textContainer.setBorder(BorderFactory.createEmptyBorder());
    ...
    /***
     * Attempts to calculate the necessary dimensions for a 
potentially wrapped
     * string of text. This isn't perfect and some extra 
whitespace above or below
     * the text may occur
     * 
     * @param str
     * @return
    ...
    private int calcHeightForText(String str, int width) {
        FontMetrics metrics = 
container.getGraphics().getFontMetrics(container.getFont());
